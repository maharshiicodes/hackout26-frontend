...
export default function LogisticsLoginComponent() {
  const router = useRouter();
  const setToken = useLogisticsAuthStore((state) => state.setToken);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const response = await apiClient.post("/api/logistics-auth/login", {
        email: emailValue,
        password: passwordValue,
      });
      setToken(response.data.token, response.data.logisticsCompanyId);
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-10">
          ...
          <div className="max-w-md">
            <h1 className="text-3xl font-semibold leading-tight text-white">
              Keep captured carbon moving.
            </h1>
            <p className="mt-3 text-sm leading-relaxed text-white/70">
              Sign in to manage your serviceable pincodes and coordinate
              shipments between buyers and sellers.
            </p>
          </div>
        </div>
      </div>
